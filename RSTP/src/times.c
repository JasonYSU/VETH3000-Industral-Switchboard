...
{
  v->MessageAge =   ntrhs (*((unsigned short*) b->message_age))   >> 8;
  v->MaxAge =       ntrhs (*((unsigned short*) b->max_age))       >> 8;
  v->ForwardDelay = ntrhs (*((unsigned short*) b->forward_delay)) >> 8;
  v->HelloTime =    ntrhs (*((unsigned short*) b->hello_time))    >> 8;
  
//  v->MessageAge =   (*((unsigned short*) b->message_age))   >> 8;
//  v->MaxAge =       (*((unsigned short*) b->max_age))       >> 8;
//  v->ForwardDelay = (*((unsigned short*) b->forward_delay)) >> 8;
//  v->HelloTime =    (*((unsigned short*) b->hello_time))    >> 8;
