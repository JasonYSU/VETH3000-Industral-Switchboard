...
static RSTP_BPDU_T bpdu_packet  = {
  {/* MAC_HEADER_T */
	...
	{0x12,0x34,0x00,0x00,0x40, 0x00, 0x00, 0x00} /* tag_switch */ 
  },
  { /* ETH_HEADER_T */
    {0x00, 0x00},               /* len8023 */
                   ...
                   unsigned short pkt_len)
{
  ...
  len8023 = htrns ((unsigned short) (pkt_len + 3));
  //len8023 = 0x2800;// ((unsigned short) (pkt_len + 3));
...
void Print_packet()
  {
		 ...
		 printf("The BPDU packet :");
		 for(loop =0 ;loop< sizeof(RSTP_BPDU_T) ; loop++ )
        {
           printf("%x \n\r",*(( (uint8_t *)&bpdu_packet) + loop )) ;
        }
