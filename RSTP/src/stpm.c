...
//At first do some test  for the  state machine for the "ROLSEL"
#define   TEST_VAL 0x44c4

/**********************************************************************/

static STPM_T *bridges = NULL;

int funct(STATE_MACH_T * this)
  {
  	 return 1; 
  }
...
/* there is an enabled bridge with same ID :( */
  ...
  stp_trace("%s (all, start stpm)",
        "clearFDB");
#endif
  //MY_PRINT("into the flush \n");
  STP_OUT_flush_lt (0, this->vlan_id, LT_FLUSH_ONLY_THE_PORT, "start stpm");
#endif
