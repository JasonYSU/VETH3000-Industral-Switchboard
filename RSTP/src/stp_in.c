...
//int _stp_in_stpm_enable (int vlan_id, char* name,BITMAP_T* port_bmp,UID_STP_MODE_T admin_state);
					          ...
					          int* err_code)
{
  ...
  this = stpapi_stpm_find (vlan_id);                   //check if the bridge with this vlan already exit  
  if (this) { /* it had just been created :( */
    *err_code = STP_Nothing_To_Do;                     
    return this;
  }
  this = STP_stpm_create (vlan_id, name);              //new a BRIDGE 
  if (! this) { /* can't create stpm :( */
    *err_code = STP_Cannot_Create_Instance_For_Vlan;   //failed to create the instance
    return NULL;
  }
...
//        MY_PRINT("\n my CREATION IS PERFECT\n");
		    STP_stpm_delete (this);                 //port creation failed,delete the instanse 
                    ...
                    UID_STP_MODE_T admin_state)
{
  ...
  if (this->admin_state == admin_state) { /* nothing to do :) */ 
    //MY_PRINT("I am so upset to be here\n");
	return 0;										  
  }													  
	// MY_PRINT(" so happy\n");												  
...
static int
_check_stpm_config (IN UID_STP_CFG_T* uid_cfg)   //查看bridge的配置 
{
...
static void                                   //使能bridge的指定PORT,并更新该端口的初始状态 
...
void 
STP_IN_init (int max_port_index) //初始化BIRIDGE的最大端口数目 
{
...
int
STP_IN_stpm_get_cfg (IN int vlan_id, OUT UID_STP_CFG_T* uid_cfg)  //get BIRDGE cfg信息 
{
  register STPM_T* this;
...
//RSTP_CRITICAL_PATH_START;  
  this = stpapi_stpm_find (vlan_id);
  if (!this) 
	 { /* this = NULL , it had not yet been created :( */
      return STP_Vlan_Had_Not_Yet_Been_Created;
   }
  if (this->admin_state != STP_DISABLED) {
    uid_cfg->field_mask |= BR_CFG_STATE;
  }
  uid_cfg->stp_enabled = this->admin_state;    //admin_state ENABLED or DISABLED 
...
//    RSTP_CRITICAL_PATH_END;
  ...
  _conv_br_id_2_uid (&port->portPrio.root_bridge, &entry->designated_root);     //????函数在上面有调用，也有原型 
  ...
  if (DisabledPort == port->role || NonStpPort == port->role) {
    memset (&entry->designated_root, 0, sizeof (UID_BRIDGE_ID_T));  //清空entry指定rootBRIDGE信息 
    memset (&entry->designated_bridge, 0, sizeof (UID_BRIDGE_ID_T));//清空指定bridge信息 
    entry->designated_cost = 0;
    entry->designated_port = port->port_id;                         //设置指定端口为指定端口 
  }
...
//#endif
    STP_OUT_flush_lt (port_index, 0, LT_FLUSH_ONLY_THE_PORT, "disable port");
  }
  for (stpm = STP_stpm_get_the_list(); stpm; stpm = stpm->next) {
     //MY_PRINT("inplement here33!!");
...
//  RSTP_CRITICAL_PATH_START;
  ...
  for (stpm = STP_stpm_get_the_list (); stpm; stpm = stpm->next) {
    if (STP_ENABLED != stpm->admin_state) continue;  //continue :directly jumps to the next circle
...
int
STP_IN_check_bpdu_header (BPDU_T* bpdu, size_t len)  //查看BPDU的类型 
{
  unsigned short len8023;

  len8023 = ntohs (*(unsigned short*) bpdu->eth.len8023);
  //len8023 = (*(unsigned short*) bpdu->eth.len8023);
                     ...
                     IN UID_STP_CFG_T* uid_cfg)
{
  ...
  UID_STP_CFG_T old;    
	...
	uint8_t       cnt;
	PORT_T *      p_port;
//	STATE_MACH_T * machine;
  /* stp_trace ("STP_IN_stpm_set_cfg"); */
	//printf("set config in...\n\r");
...
//	  printf("stpm creat here...\n\r");  
	  this = stp_in_stpm_create (vlan_id, uid_cfg->vlan_name, port_bmp, &err_code);/*STP_IN_stpm_set_cfg*/
	  if (! this) {
 //RSTP_CRITICAL_PATH_END;
	       return err_code;
      }
//	  printf("stpm creat successful...\n\r");
  }
/*To ensure the if the list is */
#ifdef MY_DEBUG
       cnt = 0;
       this   = stpapi_stpm_find (vlan_id);
			 p_port = this->ports; 
	     for (machine = p_port->machines ; p_port ; p_port = p_port->next )
	       {
              cnt++;
					    printf("machine: ");
					    printf("%d \n\r ",cnt);
  ...
  if ((BR_CFG_STATE & uid_cfg->field_mask) &&
	...
	rc = _stp_in_stpm_enable (vlan_id, uid_cfg->vlan_name, (BITMAP_T *)uid_cfg->field_mask, uid_cfg->stp_enabled);
    //printf("rc:%d\n\r",rc);
	if (0 != rc) {
...
/***************************************************************/
int
STP_STPM_IN_create (int vlan_id, 
                     char* name, 
                     BITMAP_T* port_bmp)
{
  register STPM_T*  this;
